export function loadPlayersContent() {
    return `
        ...
        <ul class="nav nav-tabs" id="divisionTabs" role="tablist" style="display: flex; border-bottom: 1px solid #ccc;">
            <li class="nav-item" role="presentation">
                <button class="nav-link active" id="d1-tab" data-bs-toggle="tab" data-bs-target="#d1" type="button"
                    role="tab" aria-controls="d1" aria-selected="true">D1</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="d2-tab" data-bs-toggle="tab" data-bs-target="#d2" type="button" role="tab"
                    aria-controls="d2" aria-selected="false">D2</button>
            </li>
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="d3-tab" data-bs-toggle="tab" data-bs-target="#d3" type="button" role="tab"
                    aria-controls="d3" aria-selected="false">D3</button>
            </li>
        </ul>
        ...
        <div class="tab-content">
                ...
                <div class="filter-container">
                    <div class="row">
                        <div class="col-auto mb-3">
                            <select id="d1-conferenceFilter" class="form-select form-select-sm">
                            </select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d1-teamFilter" class="form-select form-select-sm">
                            </select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d1-classFilter" class="form-select form-select-sm">
                            </select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d1-positionFilter" class="form-select form-select-sm">
                            </select>
                        </div>
                    </div>
                </div>
            ...
            <div class="tab-pane fade" id="d2" role="tabpanel" aria-labelledby="d2-tab">
                <div class="filter-container">
                    <div class="row">
                        <div class="col-auto mb-3">
                            <select id="d2-conferenceFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d2-teamFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d2-classFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d2-positionFilter" class="form-select form-select-sm"></select>
                        </div>
                    </div>
                </div>
                <div class="table-responsive">
                    <table id="d2-playersTable" class="table table-dark table-hover table-striped">
                        <thead>
                            <tr>
                                <th>Name</th>
                                <th>Number</th>
            ...
            <div class="tab-pane fade" id="d3" role="tabpanel" aria-labelledby="d3-tab">
                <div class="filter-container">
                    <div class="row">
                        <div class="col-auto mb-3">
                            <select id="d3-conferenceFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d3-teamFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d3-classFilter" class="form-select form-select-sm"></select>
                        </div>
                        <div class="col-auto mb-3">
                            <select id="d3-positionFilter" class="form-select form-select-sm"></select>
                        </div>
                    </div>
                </div>
                <div class="table-responsive">
...
const csvUrls = {
    'd1': 'https://raw.githubusercontent.com/widbuntu/vbdb-info/refs/heads/main/data/ncaaw_d1_24_players.csv',
    'd2': 'https://raw.githubusercontent.com/widbuntu/vbdb-info/refs/heads/main/data/ncaaw_d2_24_players.csv',
    'd3': 'https://raw.githubusercontent.com/widbuntu/vbdb-info/refs/heads/main/data/ncaaw_d3_24_players.csv'
};

// Teams data for each division
const teamsData = {
    'd1': [
        ...
        { team_short: "St. Thomas (MN)", conference_short: "Summit", division: "D-I" },
...
// Store players data for each division
const playersData = {
    'd1': [],
    'd2': [],
    'd3': []
...
// Initialize filters for a specific division
function initializeFilters(division) {
    const conferences = [...new Set(teamsData[division].map(team => team.conference_short))].sort();
    conferences.unshift('All');

    // Update conference filter
    const conferenceFilter = document.getElementById(`${division}-conferenceFilter`);
    updateDropdown(conferenceFilter, conferences);

    // Set up event listeners
    conferenceFilter.addEventListener('change', () => updateDependentFilters(division, 'conference'));
    document.getElementById(`${division}-teamFilter`).addEventListener('change', () => updateDependentFilters(division, 'team'));
    document.getElementById(`${division}-classFilter`).addEventListener('change', () => updateDependentFilters(division, 'class'));
    document.getElementById(`${division}-positionFilter`).addEventListener('change', () => updateDependentFilters(division, 'position'));

    // Initialize dependent filters
    updateDependentFilters(division, 'conference');
}
...
// Get filtered data for a division
function filterData(division) {
    const conference = document.getElementById(`${division}-conferenceFilter`).value;
    const team = document.getElementById(`${division}-teamFilter`).value;
    const playerClass = document.getElementById(`${division}-classFilter`).value;
    const position = document.getElementById(`${division}-positionFilter`).value;

    let filteredData = playersData[division];

    if (conference !== 'All') {
        filteredData = filteredData.filter(player =>
            teamsData[division].find(t => t.team_short === player.team_short)?.conference_short === conference
        );
    }

    if (team !== 'All') {
        filteredData = filteredData.filter(player => player.team_short === team);
    }

    if (playerClass !== 'All') {
        filteredData = filteredData.filter(player => player.Class === playerClass);
    }
...
// Update dependent filters for a division
function updateDependentFilters(division, changedFilter) {
    const filteredData = filterData(division);

    if (changedFilter === 'conference') {
        const conference = document.getElementById(`${division}-conferenceFilter`).value;
        let availableTeams = ['All'];
        if (conference === 'All') {
            availableTeams = [...new Set(teamsData[division].map(t => t.team_short))].sort();
            availableTeams.unshift('All');
        } else {
            availableTeams = [...new Set(teamsData[division]
                .filter(t => t.conference_short === conference)
                .map(t => t.team_short))].sort();
            availableTeams.unshift('All');
        }
        updateDropdown(document.getElementById(`${division}-teamFilter`), availableTeams);
    }

    if (changedFilter === 'conference' || changedFilter === 'team') {
        const classes = [...new Set(filteredData.map(player => player.Class))].sort();
        classes.unshift('All');
        updateDropdown(document.getElementById(`${division}-classFilter`), classes);
    }

    if (changedFilter !== 'position') {
        const positions = [...new Set(filteredData.map(player => player.Position))].sort();
        positions.unshift('All');
        updateDropdown(document.getElementById(`${division}-positionFilter`), positions);
    }

    updateTable(division, filteredData);
}

// Update table for a division
function updateTable(division, data) {
    const tbody = document.querySelector(`#${division}-playersTable tbody`);
    tbody.innerHTML = '';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    tabs.forEach(tab => {
        tab.addEventListener('shown.bs.tab', (event) => {
            const division = event.target.getAttribute('aria-controls');
            if (!playersData[division].length) { // Check if data for this division has been loaded
                initializeDivisionData(division);
            }
        });
    });
});
...
export function initPlayers() {
    ...
    ['d1', 'd2', 'd3'].forEach(division => {
        initializeDivisionData(division); // Provided function to fetch and load data
    });
    ...
    tabs.forEach(tab => {
        tab.addEventListener('shown.bs.tab', (event) => {
            const division = event.target.getAttribute('aria-controls');
            if (!playersData[division].length) {
                initializeDivisionData(division); // Load data only when tab is active
            }
        });
    });
}
